...
simple_model = SimpleModel(
    str_field="Test Message", int_field=1, float_field=1.0, bool_field=True
)
...
def test_http_serialization_handler_json_failure_headers_parsing(
    ...
    def response_func(*args, **kwargs):
        malformed_simple_model = SimpleModel(
        ...
        return HTTPResponse(
            status_code=200, headers=malformed_simple_model, data=nested_model
        )
...
def test_http_serialization_handler_json_failure_payload_parsing(
    ...
    def response_func(*args, **kwargs):
        malformed_nested_model = NestedModel(
            skip_validation=True,
            str_field=object(),
            int_field=object(),
            float_field=object(),
        ...
        return HTTPResponse(
            status_code=200, headers=simple_model, data=malformed_nested_model
        )

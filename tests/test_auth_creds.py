...
@pytest.mark.skipif(JWT_INSTALLED,
                    reason='requires that pyjwt is not installed')
...
def test_jwt_validator_pyjwt_not_installed():
    with pytest.raises(ImportError) as ex:
        jwt_validator = JWTValidator(key_or_secret='', algorithm='') # pylint: disable=unused-variable
    
    assert 'pyjwt' in str(ex.value)

@jwt_installed
@pytest.mark.skipif(pkgutil.find_loader("cryptography") != None,
...
@jwt_installed
@pytest.mark.skipif(pkgutil.find_loader("Crypto") != None,
                    reason='requires that pycrypto is not installed')
def test_jwt_validator_pycrypto_not_installed():
    with pytest.raises(ImportError) as ex:
        jwt_validator = JWTValidator(key_or_secret='', # pylint: disable=unused-variable
                                     algorithm='RS256',
                                     use_pycrypto=True)
...
@jwt_installed
@pytest.mark.skipif(pkgutil.find_loader("ecdsa") != None,
                    reason='requires that ecdsa is not installed')
...
def test_jwt_validator_valid_jwt():
    jwt_validator = JWTValidator(key_or_secret=HS256_SECRET,
                                 algorithm='HS256',
                                 verify_iss=True,
                                 issuer=ISSUER,
                                 verify_aud=True,
...
def test_jwt_validator_invalid_jwt():
    jwt_validator = JWTValidator(key_or_secret='my-secret',
                                 algorithm='HS256',
                                 verify_iss=True,
                                 issuer='wrong-issuer',
                                 verify_aud=True,
...
def test_jwt_validator_pycrypto_valid_jwt():
    jwt_validator = JWTValidator(key_or_secret=RS256_PUBLIC_KEY,
                                 algorithm='RS256',
                                 verify_iss=True,
                                 issuer=ISSUER,
                                 verify_aud=True,
...
def test_jwt_validator_ecdsa_valid_jwt():
    jwt_validator = JWTValidator(key_or_secret=ES256_PUBLIC_KEY,
                                 algorithm='ES256',
                                 verify_iss=True,
                                 issuer=ISSUER,
                                 verify_aud=True,

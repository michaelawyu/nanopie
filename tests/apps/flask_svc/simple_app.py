...
INVALID_INPUT_RES = HTTPResponse(
    status_code=400,
    data="Input is not valid."
)
USER_NOT_EXIST_RES = HTTPResponse(
    status_code=400,
...
@micro_svc.create(name='create_user',
                  rule="/users",
                  data_cls=User)
def create_user():
...
@micro_svc.update(name='update_user',
                  rule='/users/<int:uid>',
...
def update_user(uid):
    ...
    for mask in update_user_req.masks:
        if getattr(user, mask, ATTR_NOT_FOUND_FLAG) != ATTR_NOT_FOUND_FLAG and \
           getattr(new_user, mask, ATTR_NOT_FOUND_FLAG) != ATTR_NOT_FOUND_FLAG:
...
@micro_svc.list(name='list_users',
                rule='/users',
...
def list_users():
    ...
    list_users_res = [ User.from_dikt(user_data) for user_data in dummy_storage[:page_size + 1] ]
    return list_users_res


@micro_svc.custom(name='verify_user',
                  rule='/users/<int:uid>',
                  verb='verify',

...
class HTTPResponse(RPCResponse):
    """
    """
    __slots__ = ('_status_code', '_headers', '_mime_type',
                 '_data')
    ...
    def status_code(self) -> int:
        ...
        if type(self._status_code) != int:
            raise RuntimeError('HTTP Response must have a status code of '
                               'the int type.')
    ...
    def headers(self, headers: Optional[Dict]):
        ...
        if type(headers) != dict:
            raise RuntimeError('HTTP Response must have a dict as '
                               'headers.')
    ...
    def data(self) -> Optional[Any[str, bytes]]:
        ...
        if type(self._data) not in [str, bytes]:
            raise RuntimeError('HTTP Response must have a str or bytes as '
...
class HTTPService(RPCService):
    ...
    def _rest_endpoint(self,
        ...
        """
        """
        entrypoint = HTTPFoundationHandler(
    ...
    def create(self,
        ...
        """
        """
        return self._rest_endpoint(name=name,
                                   rule=rule,

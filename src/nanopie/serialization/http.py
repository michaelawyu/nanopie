...
class HTTPSerializationHandler(SerializationHandler):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if mime_type.lower() != helper.mime_type.lower():
            message = ('The incoming request does not have the expected '
                       'mime_type.')
            message = format_error_message(message=message,
                                           provided_mime_type=mime_type,
                                           expected_mime_type=helper.mime_type)
            raise SerializationError(message,
                                     response=INVALID_MIME_TYPE_RESPONSE)
        ...
        if self._headers_cls:
            try:
                ...
                message = ('The incoming request does not have '
                           'valid headers ({}).').format(str(ex))
                raise SerializationError(message,
                                         response=INVALID_HEADERS_RESPONSE)     
        ...
        if self._query_args_cls:
            try:
                ...
                message = ('The incoming request does not have '
                           'valid URI query arguments ({}).').format(str(ex))
                raise SerializationError(message,
        ...
        if self._data_cls:
            try:
                ...
                message = ('The incoming request does not have '
                           'valid body data ({}).').format(str(ex))
                raise SerializationError(message,
                                         response=INVALID_DATA_RESPONSE)

        parsed_request = HTTPParsedRequest(headers=headers,
        ...
        if isinstance(res, HTTPResponse):
            ...
            if isinstance(res.headers, Model):
                try:
                    ...
                    message = ('Cannot serialize the headers in the response. '
                               '({})').format(str(ex))
                    raise SerializationError(message)
            if isinstance(res.data, Model):
                try:
                    ...
                    message = ('Cannot serialize the data in the response. '
                               '({})').format(str(ex))
                    raise SerializationError(message)

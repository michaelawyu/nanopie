...
class OpenTelemetryTracingHandler(Handler):
    ...
    def __init__(self,
        ...
        if propagated and not trace_ctx_extractor:
            raise ValueError('trace_ctx_extractor is required '
                             'for enabling propagation.')
        self._trace_ctx_extractor = trace_ctx_extractor
        self._quiet = quiet

        self._processor = BatchExportSpanProcessor if batched \
            else SimpleExportSpanProcessor
        
        self._span_processers = []
        if write_to_console:
            self._span_processers.insert(0,
                self._processor(ConsoleSpanExporter))
            
        if with_sampler and probability:
            raise RuntimeError('with_sampler and probability are '
                               'mutually exclusive.')

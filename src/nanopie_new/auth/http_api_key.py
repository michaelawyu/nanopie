...
INVALID_HEADER_RESPONSE = HTTPResponse(
    ...
    headers={},
    mime_type='text/html',
    text_data=('<h2>401 Unauthorized: must include an API key in the '
               'request header.</h2>')
)
INVALID_QUERY_ARGS_RESPONSE = HTTPResponse(
    ...
    headers={},
    mime_type='text/html',
    text_data=('<h2>401 Unauthorized: must include an API key '
...
INVALID_TOKEN_RESPONSE = HTTPResponse(
    ...
    headers={},
    mime_type='text/html',
    text_data=('<h2>403 Forbidden: the provided API key is not valid '
               '</h2>')
)
...
class HTTPAPIKeyExtractor(CredentialExtractor):
    ...
    def extract(self, request: 'HTTPRequest'):
        ...
        if self.mode == HTTPAPIKeyModes.HEADER:
            ...
            if not auth_header:
                message = ('The incoming request does not have an API key in '
                           'the request header.')
            ...
            if not key:
                message = ('The incoming request does not have an API key '
                           'in the query string.')
                raise AuthenticationError(message,
